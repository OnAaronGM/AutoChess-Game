from functions import common_vars, conditions, classes
import numpy as np
...
def fight(pokemon_attacker: classes.Pokemon, pokemon_defender: classes.Pokemon):
...
def calcular_opciones_mov(table: list, id_user: int, pos_ini):
    ...
    for key, value in dict_poss_moves_from_ini.items():
        ...
        if type(table[key[0]][key[1]]) == classes.Pokemon and table[key[0]][key[1]].id_user != id_user:
        ...
        ## Si posición libre, se ven los movimientos adyacentes.
            ...
            for moves_2 in value:
                ...
                if type(table[moves_2[0]][moves_2[1]]) == str or \
                    (type(table[moves_2[0]][moves_2[1]]) == classes.Pokemon and table[moves_2[0]][moves_2[1]].id_user != id_user):
...
def move_unidad(table: list, player_att: classes.Player, player_def: classes.Player):
...
def sacar_unidad(table: list, player: classes.Player, turno: int) -> list:
