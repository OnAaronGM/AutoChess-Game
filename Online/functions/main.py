import random
import numpy as np
from functions import actions, utils, common_vars, classes, queries
...
def doAction(table: list, player_att: classes.Player, player_def: classes.Player, turno: int) -> bool:
    finish_turn = False
...
def jugar_partida():
    ...
    p1 = classes.Player(id=1, team=[classes.Pokemon(*unity + (1,)) for unity in queries.select_team(1)])
    p2 = classes.Player(id=2, team=[classes.Pokemon(*unity+(2,)) for unity in queries.select_team(2)])
